...
template<typename T, size_t N> struct vN_base;
...
struct vect : vect_base<T,N> {
    ...
    static constexpr bool is_simd = N == 4 && std::is_same<T,float>::value;
    ...
    T operator[](int idx) const {return this->data[idx];}

    vect operator+=(vect v) {
        if constexpr(is_simd) this->pack = _mm_add_ps(this->pack, v.pack);
        else for(size_t i = 0; i < N; i++) this->data[i] += v.data[i]; return *this;}
    vect operator-=(vect v) {
    ...
    vect operator+=(T s) {
        ...
        else for(size_t i = 0; i < N; i++) this->data[i] += s; return *this;}
    vect operator-=(T s) {
    ...
    vect operator+(vect o) const {
        ...
        else {vect r; for(size_t i = 0; i < N; i++) r.data[i] = this->data[i] + o.data[i]; return r;}}
    vect operator-(vect o) const {
    ...
    vect operator+(T s) const {
        ...
        else {vect r; for(size_t i = 0; i < N; i++) r.data[i] = this->data[i] + s; return r;}}
    vect operator-(T s) const {
    ...
    bool operator==(vect o) const {
        if constexpr(is_simd) return _mm_movemask_ps(_mm_cmpeq_ps(this->pack, o.pack)) == 0xf;
            ...
            for(size_t i = 0; good && i < N; i++) good = good && this->data[i] == o.data[i];
            return good;
        }
    }
    bool operator!=(vect o) const {return !(*this == o);}

    vect abs() const {vect r; for(size_t i = 0; i < N; i++) r.data[i] = abs(this->data[i]); return r;}
    vect operator-() {
        if constexpr(is_simd) this->pack = _mm_sub_ps(_mm_set1_ps(0.0f), this->pack);
        else for(size_t i = 0; i < N; i++) this->data[i] = -this->data[i]; return *this;}
    ...
    vect normalize() {
        ...
        else for(size_t i = 0; i < N; i++) this->data[i] /= l;
        return *this;
    }
    vect normalized() const {
        ...
        else for(size_t i = 0; i < N; i++) r.data[i] = this->data[i] / l;
        return r;
    }
...
vect<T,N> min(vect<T,N> x, vect<T,N> y) {
    ...
    vect<T,N> r;
    for(size_t i = 0; i < N; i++) r.data[i] = _MIN(x.data[i], y.data[i]);
    return r;
}
...
vect<T,N> max(vect<T,N> x, vect<T,N> y) {
    ...
    vect<T,N> r;
    for(size_t i = 0; i < N; i++) r.data[i] = _MAX(x.data[i], y.data[i]);
    return r;
}

template<typename T, size_t N>
T dot(vect<T,N> x, vect<T,N> y) {
    if constexpr(vect<T,N>::is_simd) return vect<float,4>{_mm_dp_ps(x.pack, y.pack, 0xf1)}.x;
    T r = {};
    for(size_t i = 0; i < N; i++) r += x.data[i] * y.data[i];
    return r;
}
...
using v2 = vect<float, 2>;
using v3 = vect<float, 3>;
using v4 = vect<float, 4>;

using v2i = vect<int, 2>;
using v3i = vect<int, 3>;
using v4i = vect<int, 4>;
...
struct m4 {
    ...
    m4 operator+=(m4 v) {for(int i = 0; i < 4; i++) pack[i] = _mm_add_ps(pack[i], v.pack[i]); return *this;}
    m4 operator-=(m4 v) {for(int i = 0; i < 4; i++) pack[i] = _mm_sub_ps(pack[i], v.pack[i]); return *this;}

    m4 operator+=(float s) {__m128 add = _mm_set1_ps(s); 
        for(int i = 0; i < 4; i++) pack[i] = _mm_add_ps(pack[i], add); 
        return *this;}
    m4 operator-=(float s) {__m128 sub = _mm_set1_ps(s); 
        for(int i = 0; i < 4; i++) pack[i] = _mm_sub_ps(pack[i], sub); 
        return *this;}    
    m4 operator*=(float s) {__m128 mul = _mm_set1_ps(s); 
        for(int i = 0; i < 4; i++) pack[i] = _mm_mul_ps(pack[i], mul); 
        return *this;}
    m4 operator/=(float s) {__m128 div = _mm_set1_ps(s); 
        for(int i = 0; i < 4; i++) pack[i] = _mm_div_ps(pack[i], div); 
        return *this;}
    
    v4& operator[](int idx) {return col[idx];}
    v4 operator[](int idx) const {return col[idx];}

    m4 operator+(m4 m) const {
        m4 ret;
        for(int i = 0; i < 4; i++) ret.pack[i] = _mm_add_ps(pack[i], m.pack[i]); 
        return ret;
    }
    m4 operator-(m4 m) const {
        m4 ret;
        for(int i = 0; i < 4; i++) ret.pack[i] = _mm_sub_ps(pack[i], m.pack[i]); 
        return ret;
    }

    m4 operator+(float s) const {
        m4 ret; __m128 add = _mm_set1_ps(s);
        for(int i = 0; i < 4; i++) ret.pack[i] = _mm_add_ps(pack[i], add); 
        return ret;
    }
    m4 operator-(float s) const {
        m4 ret; __m128 sub = _mm_set1_ps(s);
        for(int i = 0; i < 4; i++) ret.pack[i] = _mm_sub_ps(pack[i], sub); 
        return ret;
    }
    m4 operator*(float s) const {
        m4 ret; __m128 mul = _mm_set1_ps(s);
        for(int i = 0; i < 4; i++) ret.pack[i] = _mm_mul_ps(pack[i], mul); 
        return ret;
    }
    m4 operator/(float s) const {
        m4 ret; __m128 div = _mm_set1_ps(s);
        for(int i = 0; i < 4; i++) ret.pack[i] = _mm_div_ps(pack[i], div); 
        return ret;
    }
...
inline m4 project(float fov, float ar, float n) {
...
inline m4 rotate(float a, v3 axis) {
	m4 ret;
	float c = cosf(Radians(a));
	float s = sinf(Radians(a));
	axis = normalize(axis);
	v3 temp = axis * (1.0f - c);
